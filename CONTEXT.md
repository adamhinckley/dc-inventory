...
`packages/shared-kernel` (`Money`, `Sku`, branded IDs) is in-tree. Fastify lives in `apps/api` with stub `/internal`, `/wholesale`, and `/ops` routes. `pnpm gen:api` writes committed OpenAPI YAML and Orval clients (`packages/api-client-*`). Next.js apps are not in this pass.
