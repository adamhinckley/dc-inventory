...
import { UncoveredInventoryListQuery } from "../uncovered-inventory-list-query.js";
...
async function execUncoveredListQuerySchema(client: PGlite): Promise<void> {
  await client.exec(`
...
export async function createUncoveredListQueryPgliteHarness() {
  const client = new PGlite();
  await execUncoveredListQuerySchema(client);
  ...
  const uncoveredList = new UncoveredInventoryListQuery(db);
...
export const uncoveredListQueryPgliteIds = {
  org: ORG,
