...
export class DrizzlePaymentsReceivedListQuery implements IPaymentsReceivedListQuery {
  ...
  async list(query: PaymentsReceivedListQuery): Promise<PaymentsReceivedListPage> {
    ...
    const appliedByPayment = this.db
      .select({
    ...
    const appliedCentsExpr = sql<number>`coalesce(${appliedByPayment.appliedCents}, 0)`;
    ...
    const rows = await this.db
      ...
      .from(payments)
      .innerJoin(customers, customerJoin())
      .leftJoin(appliedByPayment, eq(appliedByPayment.paymentId, payments.id))
      .where(where)
      .orderBy(...paymentListOrderBy(query))
      .limit(query.pageSize)
      .offset(offset);

    const items: PaymentReceivedRow[] = rows.map((row) => ({
