...
describe("internal sales orders HTTP", () => {
  ...
  it("replace-lines rejects confirmed orders", async () => {
    const { app, unitOfWork } = await startSalesApp();
    const cookie = await staffCookie(app);
    ...
    await unitOfWork.run(async (scope) => {
      const result = await new RecordAdjustmentIncreaseUseCase(scope.inventory.ledger).execute({
        organizationId: OrganizationId.DEFAULT,
        idempotencyKey: "replace-confirmed-seed",
        sku: SKU,
        quantity: 10,
        refType: "adjustment",
        refId: "replace-confirmed-seed",
      });
      expect(result.ok).toBe(true);
    });
    ...
    const confirmed = await app.inject({
      ...
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
      payload: { idempotencyKey: "replace-confirmed", shipToId: API_TEST_SHIP_TO_ID },
    });
    expect(confirmed.statusCode).toBe(200);
    ...
    const replaced = await app.inject({
      ...
      url: `/internal/sales-orders/${orderId}`,
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
      payload: { lines: [{ productId: PRODUCT_ID, qty: 2 }] },
    });
