...
async function startUncoveredApp(options?: {
  ...
  const uncoveredReorderPolicy = new InMemoryPreOrderReorderPolicyReadPort();
  ...
  const app = await buildApp({
    ...
    unitOfWork,
    preOrderReorderPolicyRead: uncoveredReorderPolicy,
    supplierProductRepo,
    supplierSkuMapping,
...
describe("internal toOrder SKUs HTTP", () => {
  it("requires staff_session and lists toOrder rows with stock context", async () => {
    const app = await startUncoveredApp();
    const missing = await app.inject({ method: "GET", url: "/internal/pre-order-skus" });
  ...
  it("requires staff_session to draft purchase orders from toOrder SKUs", async () => {
    const app = await startUncoveredApp({ withDraftSuppliers: true });
  ...
  it("lists factory summary rows with a needs-mapping bucket", async () => {
    ...
    const listed = await app.inject({
      method: "GET",
      url: "/internal/pre-order-skus/factories",
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
    });
    expect(listed.statusCode).toBe(200);
    expect(listed.json()).toEqual({
  ...
  it("omits factories with open draft POs when excludeSuppliersWithOpenDraft is true", async () => {
    ...
    const filtered = await app.inject({
      method: "GET",
      url: "/internal/pre-order-skus/factories?excludeSuppliersWithOpenDraft=true",
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
    });
  ...
  it("filters enriched SKU rows by supplier and needsMapping and surfaces draft PO refs", async () => {
    ...
    const bySupplier = await app.inject({
      method: "GET",
      url: `/internal/pre-order-skus?supplierId=${SUPPLIER_A}`,
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
    });
    ...
    const needsMapping = await app.inject({
      method: "GET",
      url: "/internal/pre-order-skus?needsMapping=true",
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
    });
  ...
  it("syncs open draft PO lines from current toOrder SKUs", async () => {
    const app = await startUncoveredApp({ withDraftSuppliers: true });
    const cookie = await staffCookie(app);
    ...
    const synced = await app.inject({
      method: "POST",
      url: "/internal/purchase-orders/sync-from-pre-order",
      cookies: { [STAFF_SESSION_COOKIE]: cookie },
      payload: {},
    });
  ...
  it("drafts POs for suppliers without poPrefix using a fallback document number", async () => {
    ...
    expect(
      ...
      { id: PRE_ORDER_NEEDS_MAPPING_FACTORY_ROW_ID, poPrefix: null },
    ]);
    const drafted = await app.inject({
      method: "POST",
      url: "/internal/pre-order-skus/draft-purchase-orders",
