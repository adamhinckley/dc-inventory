import type { IPreOrderCaseQtyReadPort, IPreOrderListQuery } from "@dc-inventory/inventory";
import {
  OrganizationId,
...
export type SyncDraftPurchaseOrdersFromPreOrderRequest = {
  organizationId: OrganizationId;
  staffUserId: StaffUserId;
  supplierIds?: readonly SupplierId[];
};

export type SyncDraftPurchaseOrdersFromPreOrderResult =
  | {
      ok: true;
...
export class SyncDraftPurchaseOrdersFromPreOrderUseCase {
  constructor(
  ...
  async execute(
    input: SyncDraftPurchaseOrdersFromPreOrderRequest,
  ): Promise<SyncDraftPurchaseOrdersFromPreOrderResult> {
    const allPreOrder = await this.preOrderList.listAll({
      organizationId: input.organizationId,
    });
    const skus = allPreOrder.map((row) => row.sku);
    ...
    const grouped = groupPreOrderSkusBySupplier({
      lines: allPreOrder.map((row) => ({
        sku: row.sku,
        qty: draftPoQtyFromToOrder(
          row.toOrder,
