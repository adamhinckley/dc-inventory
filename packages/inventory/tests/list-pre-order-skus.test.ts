...
import type { IPreOrderListQuery } from "../src/domain/ports/pre-order-list-query.js";
import {
...
import {
  ...
  InMemoryPreOrderSkuSupplierMappingReadPort,
...
import { computeToOrder } from "../src/domain/demand-model.js";
...
function harness() {
  ...
  const supplierMapping = new InMemoryPreOrderSkuSupplierMappingReadPort();
  const preOrderList = new InMemoryPreOrderListQuery(h.readModel, { supplierMapping });
  const caseQty = new InMemoryPreOrderCaseQtyReadPort();
  const reorderPolicies = new InMemoryPreOrderReorderPolicyReadPort();
...
describe("List toOrder SKUs — demand-to-PO query (ADA-180)", () => {
    ...
    expect(result.items).toEqual([
      {
        ...
        onHand: 0,
        onOrder: 0,
        toOrder: computeToOrder(1_200, 0, 0),
        caseQty: null,
        reorderMin: null,
        reorderMax: null,
        ...unmappedEnrichment,
  ...
  it("lists SKUs whose code contains a colon", async () => {
    ...
    expect(result.items[0]).toMatchObject({
      ...
      onHand: 0,
      onOrder: 0,
      toOrder: computeToOrder(800, 0, 0),
    });
  });
  ...
  it("joins catalog case qty and reorder min/max without changing toOrder math", async () => {
    const h = harness();
    const commit = await h.committed({
      organizationId: DEFAULT_ORG,
    ...
    expect(result.items[0]).toEqual({
      ...
      onHand: 0,
      onOrder: 0,
      toOrder: computeToOrder(500, 0, 0),
      caseQty: 100,
      reorderMin: 24,
      reorderMax: 120,
      ...unmappedEnrichment,
