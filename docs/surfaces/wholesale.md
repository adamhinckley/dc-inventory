...
| Contact / register | UI only today. Feedback uses mailto. **Destination (locked):** public apply stores a **Pending application** (not a `Customer`); buyer sees pending success — [`customers.md`](../customers.md) §15. Staff review at internal `/customers/applications`; approve creates active customer + cert jurisdiction/number + wholesale login + invite. Staff-for-them (admin) skips pending. Header **terms** are payment clock, not a legal wholesale agreement. Live SoloView’s three-step modal + PandaDoc is legacy observation only — not this repo’s pattern (prototype PR #305 is throwaway). |
