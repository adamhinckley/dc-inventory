...
Do **not** run `inventory` and another agent that edits `packages/inventory` or `packages/shared-kernel` at the same time. Do **not** run `tax` in parallel with Sales confirm or Accounting invoice slices. Prefer at most two cheap agents in parallel (e.g. `catalog` + `customers`).
