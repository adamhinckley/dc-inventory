...
| Operation | Role |
|---|---|
| `GET /wholesale/catalog` | Browse: `q`, category, pagination, sort (shop-relevant). Default `availableOnly=true` lists sellable SKUs only (open: warehouse `available` > 0; locked: `availableToSell` > 0). Response includes image URLs, wholesale price, `available`, `availableToSell`, and `sellState` for shop sellability display. |
