...
Out of scope for Accounting: general ledger, inventory asset valuation, AP bills from POs, tax engines, multi-currency beyond storing `Money.currency`, and **software subscription** (that is [`licensing.md`](./licensing.md)).

A PO is a **purchasing document**, not a journal entry. A Stripe charge for the app itself is a **Licensing** `SoftwarePayment`, not an Accounting payment.
...
8. Sales: draft order → confirm (allocate) → ship; **wholesale shop** (catalog, cart, checkout)
9. Accounting: invoice from order + payment application (gated); invoice PDF
...
Do not start Sales allocation before Inventory tests exist. Do not start invoicing before Sales confirm exists. Do not mix Licensing `SoftwarePayment` into Accounting. Do not start Stripe until `ISoftwareBillingGateway` + payment-history tests exist.
