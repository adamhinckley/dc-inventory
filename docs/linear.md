...
All Linear **projects** and **issues** for this product — including those created from **Cursor** — belong on the **DC Inventory** initiative:
...
Do not create a Cursor/Linear project or issue for `dc-inventory` outside this initiative.
...
3. **Team.** Create issues on the **Adam Hinckley** team unless a ticket says otherwise.

Existing work on the initiative (for example the **Database mapping** project) is the default home for related issues. Create a new project on the same initiative only when the work is a distinct outcome, not a one-off ticket.
