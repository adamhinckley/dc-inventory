...
| [Procurement hub](https://linear.app/adamhinckley/project/procurement-hub-d67ec358231d) | One `/procurement` page: Pre-order, Purchase Orders, Receiving, Suppliers. Collapses Purchasing + Purchasing 2.0; Receiving leaves the sidebar. Staff language: Pre-order / To Order; Pre-sold = committed. No Uncovered. No redirects. Map: [ADA-367](https://linear.app/adamhinckley/issue/ADA-367/procurement-hub-implementation-map). API `uncovered*` rename is [ADA-373](https://linear.app/adamhinckley/issue/ADA-373/fast-follow-rename-uncovered-api-and-code-identifiers-to-pre-order-to). |
| [Order release — Pick lists](https://linear.app/adamhinckley/project/order-release-pick-lists-488a638175c0) | SoloView-parity Calculate → Release → pick in progress; iPad + printable pick lists with barcodes. Line \"Picked\" marks are visual helpers only (v1). Map: [ADA-374](https://linear.app/adamhinckley/issue/ADA-374/order-release-pick-lists-implementation-map). Grilling first; no `ready-for-agent` until G5 / picking-bucket / Calculate close. |
| [Wholesale — order history](https://linear.app/adamhinckley/project/wholesale-order-history-19d63c6943ef) | Shop `/orders` history and `/orders/SO-xxxxx` detail. Map: [ADA-402](https://linear.app/adamhinckley/issue/ADA-402/wholesale-order-history-implementation-map). HTTP is [ADA-403](https://linear.app/adamhinckley/issue/ADA-403/get-wholesale-sales-order-by-document-number); UI is [ADA-404](https://linear.app/adamhinckley/issue/ADA-404/wholesale-orders-list-links-and-ordersso-xxxxx-detail). Layout prototypes: [ADA-405](https://linear.app/adamhinckley/issue/ADA-405/prototype-wholesale-order-history-list-and-detail-layouts). |
| [Identity — login launch mitigations](https://linear.app/adamhinckley/project/identity-login-launch-mitigations-2d0ae9d521dd) | Launch gate: reset loop, idle kick, login-method hint, org slug UX, inactive vs invalid-password. Map: [DCI-400](https://linear.app/adamhinckley/issue/DCI-400/identity-login-launch-mitigations-map). Identity stays SoT. Not `ready-for-agent` until Given exists. |
| [Onboarding — three tiers](https://linear.app/adamhinckley/project/onboarding-three-tiers-620f6bb53aa7) | (1) Adam onboards a business/`Organization`. (2) First staff is the David-shaped super user (`admin`, not a sixth role). (3) That person onboards staff (G8 roles) and customers (Customer + shop login). Map: [DCI-406](https://linear.app/adamhinckley/issue/DCI-406/onboarding-three-tiers-map). |
